...
export class Wallet {
   ...
   *
   * `accountAddress` is the account's ORIGINAL, STABLE address -- the one that holds the balance.
   * It is deliberately NOT derived from the key, because after a rotation the key derives a
   * different, empty address; a wallet that adopted the derived address would have silently
   * abandoned the account. Verify with `readKeyRegistry(provider, accountAddress)` that
   * `currentKeyHash === keyHashOf(publicKey)` before using this.
   */
