// src/wallet.ts
import { loadWasm, MlKemNode } from './webassembly/mlkem-node';
import type { Provider } from './provider';
import { serializeForRpc } from './utils.js';
...
export class Wallet {
  private mlkem: MlKemNode;
  public privateKey: string;
  public readonly address: string;

  private constructor(mlkem: MlKemNode, privateKey: string) {
    this.mlkem = mlkem;
...
export class Signer {
  ...
  async sendTransaction(txParams: TxParams): Promise<string> {
    ...
    return sendResponse.result as string; // returns tx hash
  }
}
