  ...
  function createKyber(opts2) {
    ...
    return {
      ...
      encapsulate: (publicKey, msg) => {
        ...
        //chunk the message into 32bit chunks
        var msgChunks=[];
        for (let i = 0; i < msg.length; i += 32) {
            let msgChunk = msg.slice(i, i + 32);
            // If block is less than blockSize, pad it (here using zero padding)
            if (msgChunk.length < 32) {
        ...
        //encapsulate each of the chunk into a cipher text
        var cipherTexts=[];
        for(var j=0 ; j<msgChunks.length ;j++ )
        {
          var msgChunk = msgChunks[j];
          ensureBytes(msgChunk, msgLen);
          const kr = HASH512.create().update(msgChunk).update(HASH256(publicKey)).digest();
          const cipherText = KPKE.encrypt(publicKey, msgChunk, kr.subarray(32, 64));
          //push this ciphertexts into the array
        ...
        const totalLength = cipherTexts.reduce((sum, cipherText) => sum + cipherText.length, 0);
        ...
        return result;
        //concatanate the ciphertext array and return the final ciphertext

        return { cipherTexts };
      },
      decapsulate: (cipherTextsWhole, secretKey) => {
        ensureBytes(secretKey, secretKeyLen);
        const cipherTexts = [];
        for (let i = 0; i < cipherTextsWhole.length; i += 1568) {
          // Since data.length is a multiple of blockSize, each slice will be exactly blockSize bytes.
          const block = cipherTextsWhole.slice(i, i + 1568);
          cipherTexts.push(block);
        }

        //loop through the cipher key arrays 
        var msgWhole=[];
        for(var i=0;i<cipherTexts.length ; i++){
          ...
          //console.log("message after decapsulation:",(msg));
          const kr = HASH512.create().update(msg).update(publicKeyHash).digest();
          const Khat = kr.subarray(0, 32);
          const cipherText2 = KPKE.encrypt(publicKey, msg, kr.subarray(32, 64));
          const isValid = equalBytes(cipherText, cipherText2);
          const Kbar = KDF.create({ dkLen: 32 }).update(z).update(cipherText).digest();
          cleanBytes(cipherText2, !isValid ? Khat : Kbar);
          msgWhole.push(msg);
        }

        //converting the message array into single message 
        const totalLength = msgWhole.reduce((sum, msgBlock) => sum + msgBlock.length, 0);
        const result = new Uint8Array(totalLength);
        let offset = 0;
        for (const msgBlock of msgWhole) {
          result.set(msgBlock, offset);
          offset += msgBlock.length;
        }
        return result;
...
module.exports = noblePostQuantum;